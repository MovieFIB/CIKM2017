...
class OutStHtLSTMLayer(MergeLayer):
    def __init__(self, incoming, num_units,
        ...
        super(OutStHtLSTMLayer, self).__init__(incomings, **kwargs)
        ...
        self.st_init = self.add_param(
            st_init, (1, self.num_units), name="st_init",
            trainable=learn_init, regularizable=False
        )
    ...
    def get_output_shape_for(self, input_shapes):
        ...
        if self.only_return_final:
            return input_shape[0], self.num_units*2
        else:
            return input_shape[0], input_shape[1], self.num_units*2
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        def step_masked(
            input_n, mask_n,
            cell_previous, hid_previous, st_previous,
            W_hid_stacked, W_in_stacked, b_stacked,
            W_cell_to_ingate, W_cell_to_forgetgate, W_cell_to_outgate,
        ):
            cell, hid, st = step(
            ...
            st = T.switch(mask_n, st, st_previous)
        ...
        st_init = T.dot(ones, self.st_init)
        ...
        if self.unroll_scan:
            ...
            cell_out, hid_out, st_out = unroll_scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, st_init],
            ...
            cell_out, hid_out, st_out = theano.scan(
                fn=step_fun,
                sequences=sequences,
                outputs_info=[cell_init, hid_init, st_init],
                go_backwards=self.backwards,
        ...
        if self.only_return_final:
            ...
            st_out = st_out.dimshuffle(1, 0, 2)
            if self.backwards:
                st_out = st_out[:, ::-1]
                hid_out = hid_out[:, ::-1]
        out = T.concatenate(
